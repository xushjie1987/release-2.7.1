...
abstract public class Command extends Configured {
    ...
    public int runAll() {
        ...
        for (String src : args) {
            try {
                PathData[] srcs = PathData.expandAsGlob(src,
                                                        getConf());
                for (PathData s : srcs) {
                    run(s.path);
                }
     ...
     * </pre>
     * 
     * Most commands will chose to implement just {@link #processOptions(LinkedList)} and
     * {@link #processPath(PathData)}
     * @param argv
    ...
    public int run(String... argv) {
        ...
        try {
            if (isDeprecated()) {
                displayWarning("DEPRECATED: Please use '" +
    ...
    /**
     * The exit code to be returned if any errors occur during execution. This method is needed to
     * account for the inconsistency in the exit codes returned by various commands.
    ...
    /**
     * Must be implemented by commands to process the command line flags and check the bounds of the
     * remaining arguments. If an IllegalArgumentException is thrown, the FsShell object will print
     * the short usage of the command.
     * @param args
    ...
    /**
     * Allows commands that don't use paths to handle the raw arguments. Default behavior is to
     * expand the arguments via {@link #expandArguments(LinkedList)} and pass the resulting list to
     * {@link #processArguments(LinkedList)}
     * @param args
    ...
    /**
     * Expands a list of arguments into {@link PathData} objects. The default behavior is to call
     * {@link #expandArgument(String)} on each element which by default globs the argument. The loop
     * catches IOExceptions, increments the error count, and displays the exception.
     * @param args
     *            strings to expand into {@link PathData} objects
    ...
    /**
     * Expand the given argument into a list of {@link PathData} objects. The default behavior is to
     * expand globs. Commands may override to perform other expansions on an argument.
     * @param arg
     *            string pattern to expand
     * @return list of {@link PathData} objects
     * @throws IOException
     *             if anything goes wrong...
     */
    protected List<PathData> expandArgument(String arg) throws IOException {
        PathData[] items = PathData.expandAsGlob(arg,
                                                 getConf());
        if (items.length == 0) {
    ...
    /**
     * Processes the command's list of expanded arguments. {@link #processArgument(PathData)} will
     * be invoked with each item in the list. The loop catches IOExceptions, increments the error
     * count, and displays the exception.
     * @param args
     *            a list of {@link PathData} to process
     * @throws IOException
     *             if anything goes wrong...
     */
    ...
    /**
     * Processes a {@link PathData} item, calling {@link #processPathArgument(PathData)} or
     * {@link #processNonexistentPath(PathData)} on each item.
     * @param item
     *            {@link PathData} item to process
     * @throws IOException
     *             if anything goes wrong...
     */
     ...
     * {@link #processPaths(PathData, PathData...)} -> {@link #processPath(PathData)} loop. Ex. ls
     * and du use this to expand out directories.
     * @param item
     *            a {@link PathData} representing a path which exists
     * @throws IOException
     *             if anything goes wrong...
     */
    ...
    /**
     * Provides a hook for handling paths that don't exist. By default it will throw an exception.
     * Primarily overriden by commands that create paths such as mkdir or touch.
     * @param item
     *            the {@link PathData} that doesn't exist
     * @throws FileNotFoundException
     *             if arg is a path and it doesn't exist
     * @throws IOException
    ...
    /**
     * Iterates over the given expanded paths and invokes {@link #processPath(PathData)} on each
     * element. If "recursive" is true, will do a post-visit DFS on directories.
     * @param parent
     *            if called via a recurse, will be the parent dir, else null
     * @param items
     *            a list of {@link PathData} objects to process
     * @throws IOException
     *             if anything goes wrong...
     */
    protected void processPaths(PathData parent,
                                PathData... items) throws IOException {
    ...
    /**
     * Determines whether a {@link PathData} item is recursable. Default implementation is to
     * recurse directories but can be overridden to recurse through symbolic links.
     * @param item
    ...
    /**
     * Hook for commands to implement an operation to be applied on each path for the command. Note
     * implementation of this method is optional if earlier methods in the chain handle the
     * operation.
     * @param item
     *            a {@link PathData} object
     * @throws RuntimeException
     *             if invoked but not implemented
     * @throws IOException
    ...
    /**
     * Hook for commands to implement an operation to be applied on each path for the command after
     ...
     * {@link #processPaths(PathData, PathData...)}
     * @param item
     *            {@link PathData} for directory to recurse into
     * @throws IOException
     *             if anything goes wrong...
     */
    ...
    /**
     * Display an exception prefaced with the command name. Also increments the error count for the
     * command which will result in a non-zero exit code.
     * @param e
    ...
    /**
     * Display an error string prefaced with the command name. Also increments the error count for
     * the command which will result in a non-zero exit code.
     * @param message
    ...
    /**
     * The name of the command. Will first try to use the assigned name else fallback to the
     * command's preferred name
    ...
    public String getUsage() {
        String cmd = "-" + getName();
        String usage = isDeprecated()
                                     ? ""
                                     : getCommandField("USAGE");
