...
/**
 * Encapsulates a Path (path), its FileStatus (stat), and its FileSystem (fs). PathData ensures that
 * the returned path string will be the same as the one passed in during initialization (unlike Path
 * objects which can modify the path string). The stat field will be null if the path does not
 * exist.
 */
...
public class PathData implements Comparable<PathData> {
    ...
    /**
     * Creates an object to wrap the given parameters as fields. The string used to create the path
     * will be recorded since the Path object does not return exactly the same string used to
     * initialize it
     * @param pathString
     *            a string for a path
     * @param conf
    ...
    public PathData(String pathString,
        ...
        this(FileSystem.get(stringToUri(pathString),
                            conf),
             pathString);
    }
     ...
     *             if anything goes wrong...
     */
    public PathData(URI localPath,
                    Configuration conf) throws IOException {
        this(FileSystem.getLocal(conf),
             localPath.getPath());
    }
     ...
     *             if anything goes wrong
     */
    private PathData(FileSystem fs,
     ...
     * @param pathString
     *            a String of the path suppliued by the user.
     * @return true if the URI scheme was not present in the pathString but inferred; false,
     *         otherwise.
     * @throws IOException
     *             if anything goes wrong
     */
    private static boolean checkIfSchemeInferredFromPath(String pathString) throws IOException {
        if (windowsNonUriAbsolutePath1.matcher(pathString)
                                      .find()) {
        ...
        if (windowsNonUriAbsolutePath2.matcher(pathString)
                                      .find()) {
            return true;
        }
        
        // Does it look like a URI? If so then just leave it alone.
        if (potentialUri.matcher(pathString)
                        .find()) {
            return false;
        }
     ...
     * @param stat
     *            the FileStatus (may be null if the path doesn't exist)
     */
    private PathData(FileSystem fs,
                     String pathString,
     ...
     * @param ignoreFNF
     *            if true, stat will be null if the path doesn't exist
     * @return FileStatus for the given path
     * @throws IOException
     *             if anything goes wrong
     */
    private static FileStatus lookupStat(FileSystem fs,
                                         String pathString,
                                         boolean ignoreFNF) throws IOException {
     ...
     * @param typeRequirement
     *            Set it to the desired requirement.
     * @throws PathIOException
     *             if file doesn't exist or the type does not match what was specified in
     *             typeRequirement.
     */
     ...
     * @throws IOException
     *             upon unexpected error
     */
    public boolean parentExists() throws IOException {
        return representsDirectory()
                                    ? fs.exists(path)
                                    : fs.exists(path.getParent());
    }
    
    /**
     * Check if the path represents a directory as determined by the basename being "." or "..", or
     * the path ending with a directory separator
     ...
     * @param child
     *            the basename will be appended to this object's path
     * @return PathData for the child
     * @throws IOException
     *             if this object does not exist or is not a directory
     */
    public PathData getPathDataForChild(PathData child) throws IOException {
        checkIfExists(FileTypeRequirement.SHOULD_BE_DIRECTORY);
        return new PathData(fs,
                            getStringForChildPath(child.path));
    }
    
    /**
     * Given a child of this directory, use the directory's path and the child's basename to
     * construct the string to the child. This preserves relative paths since Path will fully
     * qualify.
     * @param childPath
    ...
    private String getStringForChildPath(Path childPath) {
        ...
        return uriToString(uri,
                           inferredSchemeFromPath) + separator + basename;
    }
    ...
    /**
     * Expand the given path as a glob pattern. Non-existent paths do not throw an exception because
     * creation commands like touch and mkdir need to create them. The "stat" field will be null if
     * the path does not exist.
     * @param pattern
     *            the pattern to expand as a glob
     * @param conf
     *            the hadoop configuration
     * @return list of {@link PathData} objects. if the pattern is not a glob, and does not exist,
     *         the list will contain a single PathData with a null stat
     * @throws IOException
     *             anything else goes wrong...
     */
    public static PathData[] expandAsGlob(String pattern,
                                          Configuration conf) throws IOException {
        ...
        if (stats == null) {
            ...
            items = new PathData[] { new PathData(fs,
                                                  pattern,
                                                  null) };
        } else {
            ...
            for (FileStatus stat : stats) {
                ...
                switch (globType) {
                    ...
                    case RELATIVE: // make it relative to the current working dir
                        URI cwdUri = fs.getWorkingDirectory()
                                       .toUri();
                        globMatch = relativize(cwdUri,
                                               matchUri,
                                               stat.isDirectory());
                        break;
                }
    ...
    private static String relativize(URI cwdUri,
                                     URI srcUri,
                                     boolean isDir) {
        ...
        int lastSep = findLongestDirPrefix(cwdPath,
                                           uriPath,
                                           isDir);
        ...
        if (lastSep < cwdPath.length()) {
            while (lastSep != -1) {
                ...
                relPath.insert(0,
                               "..");
                lastSep = cwdPath.indexOf(Path.SEPARATOR,
                                          lastSep + 1);
            }
        }
    ...
    private static int findLongestDirPrefix(String cwd,
                                            String path,
                                            boolean isDir) {
        ...
        int len = Math.min(cwd.length(),
                           path.length());
        int lastSep = -1;
        for (int i = 0; i < len; i++) {
            if (cwd.charAt(i) != path.charAt(i))
                break;
            if (cwd.charAt(i) == Path.SEPARATOR_CHAR)
                lastSep = i;
        }
        return lastSep;
    }
    ...
    /**
     * Returns the printable version of the path that is either the path as given on the
     * commandline, or the full path
     * @return String of the path
     */
    @Override
    public String toString() {
        return uriToString(uri,
                           inferredSchemeFromPath);
    }
    
    private static String uriToString(URI uri,
                                      boolean inferredSchemeFromPath) {
        ...
        if ((scheme == null) || (inferredSchemeFromPath)) {
            if (Path.isWindowsAbsolutePath(decodedRemainder,
                                           true)) {
     ...
     *             if this.fs is not the LocalFileSystem
     */
    public File toFile() {
    ...
    /**
     * Normalize the given Windows path string. This does the following: 1. Adds "file:" scheme for
     * absolute paths. 2. Ensures the scheme-specific part starts with '/' per RFC2396. 3. Replaces
     * backslash path separators with forward slashes.
     * @param pathString
     *            Path string supplied by the user.
     * @return normalized absolute path string. Returns the input string if it is not a Windows
     *         absolute path.
     */
    ...
    /**
     * Construct a URI from a String with unescaped special characters that have non-standard
     * semantics. e.g. /, ?, #. A custom parsing is needed to prevent misbehavior.
     * @param pathString
     *            The input path in string form
     * @return URI
     */
    private static URI stringToUri(String pathString) throws IOException {
        ...
        if (pathString.startsWith("//",
                                  start) && (pathString.length() - start > 2)) {
            start += 2;
            int nextSlash = pathString.indexOf('/',
                                               start);
            int authEnd = nextSlash > 0
                                       ? nextSlash
                                       : pathString.length();
