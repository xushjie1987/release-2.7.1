...
/**
 * <code>GenericOptionsParser</code> is a utility to parse command line arguments generic to the
 * Hadoop framework. <code>GenericOptionsParser</code> recognizes several standard command line
 * arguments, enabling applications to easily specify a namenode, a ResourceManager, additional
 * configuration resources etc. <h4 id="GenericOptions">Generic Options</h4>
 * <p>
 ...
 * </p>
 * <p>
 * Generic command line arguments <strong>might</strong> modify <code>Configuration </code> objects,
 * given to constructors.
 * </p>
 * <p>
...
public class GenericOptionsParser {
     ...
     * @throws IOException
     */
    public GenericOptionsParser(Options opts,
    ...
    /**
     * Create a <code>GenericOptionsParser</code> to parse given options as well as generic Hadoop
     * options. The resulting <code>CommandLine</code> object can be obtained by
     * {@link #getCommandLine()}.
     * @param conf
     ...
     * @return array of <code>String</code>s containing the un-parsed arguments or <strong>empty
     *         array</strong> if commandLine was not defined.
     */
     ...
     * Note: If the object is created with {@link #GenericOptionsParser(Configuration, String[])},
     * then returned object will only contain parsed generic options.
     * @return <code>CommandLine</code> representing list of arguments parsed against Options
     *         descriptor.
     */
    ...
    private void processGeneralOptions(Configuration conf,
        ...
        if (line.hasOption("libjars")) {
            ...
            if (libjars != null && libjars.length > 0) {
                ...
                Thread.currentThread()
                      .setContextClassLoader(new URLClassLoader(libjars,
                                                                Thread.currentThread()
        ...
        conf.setBoolean("mapreduce.client.genericoptionsparser.used",
                        true);
        ...
        if (line.hasOption("tokenCacheFile")) {
            ...
            UserGroupInformation.getCurrentUser()
                                .addCredentials(Credentials.readTokenStorageFile(p,
                                                                                 conf));
    ...
    public static URL[] getLibJars(Configuration conf) throws IOException {
        ...
        for (String file : files) {
            ...
            if (tmp.getFileSystem(conf)
                   .equals(FileSystem.getLocal(conf))) {
                cp.add(FileSystem.getLocal(conf)
                                 .pathToFile(tmp)
                                 .toURI()
                                 .toURL());
            } else {
                LOG.warn("The libjars file " +
                         tmp +
                         " is not on the local " +
                         "filesystem. Ignoring.");
            }
    ...
    /**
     * takes input as a comma separated list of files and verifies if they exist. It defaults for
     * file:/// if the files specified do not have a scheme. it returns the paths uri converted
     * defaulting to file:///. So an input of /home/user/file1,/home/user/file2 would return
    ...
    /**
     * Windows powershell and cmd can parse key=value themselves, because /pkey=value is same as
     * /pkey value under windows. However this is not compatible with how we get arbitrary key
     * values in -Dkey=value format. Under windows -D key=value or -Dkey=value might be passed as
     * [-Dkey, value] or [-D key, value]. This method does undo these and return a modified args
     * list by manually changing [-D, key, value] into [-D, key=value]
     * @param args
     *            command line arguments
    ...
    /**
     * Parse the user-specified options, get the generic options, and modify configuration
     * accordingly
     * @param opts
     ...
     *            User-specified arguments
     */
    private void parseGeneralOptions(Options opts,
                                     Configuration conf,
                                     String[] args) throws IOException {
        opts = buildGeneralOptions(opts);
        CommandLineParser parser = new GnuParser();
        try {
            commandLine = parser.parse(opts,
                                       preProcessForWindows(args),
                                       true);
            processGeneralOptions(conf,
                                  commandLine);
        } catch (ParseException e) {
            LOG.warn("options parsing failed: " + e.getMessage());
            
            HelpFormatter formatter = new HelpFormatter();
            formatter.printHelp("general options are: ",
                                opts);
        }
    }
