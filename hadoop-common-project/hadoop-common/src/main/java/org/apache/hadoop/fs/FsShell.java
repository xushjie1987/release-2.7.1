...
public class FsShell extends Configured implements Tool {
    ...
    /**
     * Default ctor with no configuration. Be sure to invoke {@link #setConf(Configuration)} with a
     * valid configuration prior to running commands.
     */
    public FsShell() {
        this(null);
    }
    ...
    /**
     * Construct a FsShell with the given configuration. Commands can be executed via
     * {@link #run(String[])}
     * @param conf
    ...
    protected void init() throws IOException {
        ...
        if (commandFactory == null) {
            ...
            commandFactory.addObject(new Help(),
                                     "-help");
            commandFactory.addObject(new Usage(),
                                     "-usage");
    ...
    /*
     * The following are helper methods for getInfo(). They are defined outside of the scope of the
     * Help/Usage class because the run() method needs to invoke them too.
     */
    ...
    private void printInstanceUsage(PrintStream out,
                                    Command instance) {
        out.println(usagePrefix + " " + instance.getUsage());
    }
    
    private void printInstanceHelp(PrintStream out,
                                   Command instance) {
        out.println(instance.getUsage() + " :");
        TableListing listing = null;
        final String prefix = "  ";
        for (String line : instance.getDescription()
                                   .split("\n")) {
            if (line.matches("^[ \t]*[-<].*$")) {
                ...
                if (segments.length == 2) {
                    ...
                    listing.addRow(segments[0].trim(),
                                   segments[1].trim());
                    continue;
                }
            }
            ...
            if (listing != null) {
                for (String listingLine : listing.toString()
                                                 .split("\n")) {
            ...
            for (String descLine : WordUtils.wrap(line,
                                                  MAX_LINE_WIDTH,
                                                  "\n",
                                                  true)
                                            .split("\n")) {
    ...
    private TableListing createOptionTableListing() {
        return new TableListing.Builder().addField("")
                                         .addField("",
                                                   true)
    ...
    public int run(String argv[]) throws Exception {
        ...
        if (argv.length < 1) {
            ...
            try {
                ...
                exitCode = instance.run(Arrays.copyOfRange(argv,
                                                           1,
                                                           argv.length));
    ...
    private void displayError(String cmd,
                              String message) {
    ...
    /**
     * The default ctor signals that the command being executed does not exist, while other ctor
     * signals that a specific command does not exist. The latter is used by commands that process
     * other commands, ex. -usage/-help
     */
