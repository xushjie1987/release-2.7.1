...
public class CommandFactory extends Configured {
    ...
    /**
     * Invokes "static void registerCommands(CommandFactory)" on the given class. This method
     * abstracts the contract between the factory and the command class. Do not assume that directly
     * invoking registerCommands on the given class will have the same effect.
     * @param registrarClass
     *            class to allow an opportunity to register
     */
    public void registerCommands(Class<?> registrarClass) {
        try {
            registrarClass.getMethod("registerCommands",
     ...
     */
    public void addClass(Class<? extends Command> cmdClass,
                         String... names) {
    ...
    /**
     * Register the given object as handling the given list of command names. Avoid calling this
     * method and use {@link #addClass(Class, String...)} whenever possible to avoid startup
     * overhead from excessive command object instantiations. This method is intended only for
     * handling nested non-static classes that are re-usable. Namely -help/-usage.
     * @param cmdObject
     *            the object implementing the command names
     * @param names
     *            one or more command names that will invoke this class
     */
    public void addObject(Command cmdObject,
                          String... names) {
        for (String name : names) {
            objectMap.put(name,
                          cmdObject);
            classMap.put(name,
                         null); // just so it shows up in the list of commands
        }
    }
    
    /**
     * Returns an instance of the class implementing the given command. The class must have been
     * registered via {@link #addClass(Class, String...)}
     * @param cmd
     *            name of the command
     ...
     */
    public Command getInstance(String cmdName,
                               Configuration conf) {
        if (conf == null)
        ...
        if (instance == null) {
            ...
            if (cmdClass != null) {
                instance = ReflectionUtils.newInstance(cmdClass,
                                                       conf);
